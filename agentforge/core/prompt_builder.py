...
def build_agent_generation_prompt(spec: dict, repo_meta: dict) -> str:
    ...
    return (
        ...
        "For each agent, include responsibilities that explicitly cover:\n"
        "- Inputs or source material the agent must use.\n"
        "- Output artifacts or deliverables (name files or formats when applicable).\n"
        "- A short, ordered process or decision path.\n"
        "For each agent, include constraints that explicitly cover:\n"
        "- Prohibited actions or out-of-scope work.\n"
        "- Quality gates or validation checks.\n"
        "- When to escalate, hand off, or request clarification.\n\n"
        "Output schema:\n"
        "{\n"
        '  "agents": [\n'
        "    {\n"
